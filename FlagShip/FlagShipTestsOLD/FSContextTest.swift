...
class FSContextTest: XCTestCase {
    ...
    func testUpdateContext() {
        ...
        let values: [String: Any] = ["k1": "Val1", "k2": 12, "k3": true, "k4": 0.4, "k5": 2.01212121212121212121212121212, "k6": 0.1111]

        for item in values {

            if item.value.self is Int {
                Flagship.sharedInstance.updateContext(item.key, item.value as? Int ?? 0)

            } else if item.value.self is Double {
                Flagship.sharedInstance.updateContext(item.key, item.value as? Double ?? 0.0)
