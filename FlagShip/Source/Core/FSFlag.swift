...
public class FSFlag: NSObject {
    ...
    @objc public func value(visitorExposed: Bool = true)->Any? {
        ...
        if let flagModification = strategy?.getStrategy().getFlagModification(key) {
            if isSameType_or_DefaultValue_Nil(flagModification.value){ /// _ have type same with default value OR the default value is nil
                ///
    ...
    @objc public func visitorExposed() {
        if let flagModification = strategy?.getStrategy().getFlagModification(key) {
            ...
            if flagModification.value is NSNull || isSameType_or_DefaultValue_Nil(flagModification.value) || (defaultValue == nil) {
                /// Activate the flag
