...
public extension FSFlag {
    internal func isSaferExposure() {
        var outPutMsg = ""
        if isSafeToExpose {
            ...
            if defaultValue == nil {
                ...
                // Send Troubleshooting hit
                FSDataUsageTracking.sharedInstance.proceesTSFlag(crticalPointLabel: .FLAG_EXPOSED_BEFORE_CALLING_VALUE_METHOD, f: self, v: strategy?.visitor)
    ...
    /// _ Objective-c
    @available(swift, obsoleted: 1.0, message: "For Objective_C use only")
    @objc func value(defaultValue: Any, visitorExposed: Bool = true) -> Any {
        switch defaultValue {
            case _ as String:
                /// Compare with String
