...
class FSConfigViewController: UIViewController, UITextFieldDelegate, FSJsonEditorDelegate {
    ...
    func createVisitor() -> FSVisitor {
        ...
        return Flagship.sharedInstance.newVisitor("").hasConsented(hasConsented: allowTrackingSwitch?.isOn ?? true).withContext(context: ["segment": "coffee", "QA": "ios", "testing_tracking_manager": true, "qa_report": true, "country": "FR", "condition1": "val1", "semkey": "0.2.1"]).isAuthenticated(authenticateSwitch?.isOn ?? false).build()
    }
