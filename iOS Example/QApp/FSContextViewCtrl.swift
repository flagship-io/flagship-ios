    ...
    @IBAction func onClikcUpdateCtx(){
        ...
        if keyInput.count != 0{
            ...
            if let valueInput = valueField?.text {
                ...
                switch typeValue {
                case .BooleanType:
                    Flagship.sharedInstance.sharedVisitor?.updateContext(keyInput, valueSwitch?.isOn ?? false)
                    break
                case .StringType:
                    Flagship.sharedInstance.sharedVisitor?.updateContext(keyInput,valueInput)
                    break
                case .IntegerType:
                    
                    let inputInt = Int(String(format: "%@", valueInput)) ?? 0
                    Flagship.sharedInstance.sharedVisitor?.updateContext(keyInput,inputInt)

                    break
                case .DoubleType:
                    
                    let inputDbl = Double(String(format: "%@", valueInput)) ?? 0
                    Flagship.sharedInstance.sharedVisitor?.updateContext(keyInput,inputDbl)
                ...
                DispatchQueue.main.async {
                    ...
                    if let visitor = Flagship.sharedInstance.sharedVisitor {
                        ...
                        self.currentCtx.text = String(format: "%@", visitor.getContext())
                    }
